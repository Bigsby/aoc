        ...
        for index, number in enumerate(ticket):
            for fieldName in names:
                ...
                if number < startOne or (number > endOne and number < startTwo) or number > endTwo:
                    toRemove: List[Tuple[str, int]] = []
                    positions[fieldName].remove(index)
                    if len(positions[fieldName]) == 1:
                        toRemove.append(
                            (fieldName, next(iter((positions[fieldName])))))
                    while len(toRemove):
                        ...
                        for otherFieldName in names:
                            ...
                            if len(positions[otherFieldName]) == 1:
                                toRemove.append((otherFieldName, next(
                                    iter(positions[otherFieldName]))))
    departureFieldIndexes = [next(iter(positions[fieldName]))
                             for fieldName in names if fieldName.startswith("departure")]
    result = reduce(lambda soFar, index: soFar *
                    myTicket[index], departureFieldIndexes, 1)
    return result
    ...
    with open(filePath, "r") as file:
        ...
        for line in file.readlines():
            if doingRules:
                ...
                if fieldMatch:
                    rules.append((fieldMatch.group("field"), int(fieldMatch.group("r1s")), int(
                        fieldMatch.group("r1e")), int(fieldMatch.group("r2s")), int(fieldMatch.group("r2e"))))
                else:
                    doingRules = False
