...
def getInput(filePath: str) -> Tuple[List[Record],List[Operation]]:
    ...
    with open(filePath, "r") as file:
        ...
        for match in recordRegex.finditer(recordsText):
            records.append(( \
                (int(match.group("b0")), int(match.group("b1")), int(match.group("b2")), int(match.group("b3"))), \
                (int(match.group("opCode")), int(match.group("A")), int(match.group("B")), int(match.group("C"))), \
                (int(match.group("a0")), int(match.group("a1")), int(match.group("a2")), int(match.group("a3"))) \
                ))
        operations = []
        for match in operationRegex.finditer(programText):
            operations.append((int(match.group("opCode")), int(match.group("A")), int(match.group("B")), int(match.group("C"))))
